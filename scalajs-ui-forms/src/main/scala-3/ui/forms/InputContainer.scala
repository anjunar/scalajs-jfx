...
import ui.core.dsl.ClassDsl.{addClass as addDslClass}
...
final class InputContainer(body: => (AbstractComponent ?=> Cursor ?=> Unit))
  ...
  override def compose(cursor: Cursor): Unit =
    render(this, cursor) {
      addClass("ui-input-container")

      val labelHost = div {
        addDslClass("ui-input-container__label")
        div {
          addDslClass("placeholder")
          addDslClass("ui-input-container__placeholder")
          text(labelProperty) {}
        }
      }

      val controlHost = div {
        addDslClass("ui-input-container__control")
        body
      }

      val divider = div {
        addDslClass("ui-input-container__divider")
      }

      val errorsHost = div {
        addDslClass("ui-input-container__errors")
      }
