...
import ui.core.component.{AbstractComponent, AbstractCustomComponent, DynamicMountPoint, Runtime}
import ui.core.dsl.DslLayer
import ui.core.render.{Cursor, HostNode}
...
object Foreach {
  ...
  private[ui] def foreachIndexedPreservingUpdates[V](items: ListProperty[V])(
      body: (V, Int) => AbstractComponent ?=> Cursor ?=> Unit
  )(using AbstractComponent, Cursor): Foreach[V] =
    DslLayer.child(
