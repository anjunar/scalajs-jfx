...
final class Carousel[T] private[carousel] (
  ...
  override def compose(cursor: Cursor): Unit = {
    ...
    DslLayer.render(this, cursor) {
      ...
      div {
        classes = Seq("ui-carousel__viewport")

        div {
          classes = Seq("ui-carousel__track")
      ...
      when(multipleItemsProperty) {
        div {
          classes = Seq("ui-carousel__controls")

          button("Previous") {
            classes = Seq("ui-carousel__nav")
            buttonType("button")
          ...
          div {
            classes = Seq("ui-carousel__status")
            setAttribute("aria-live", "polite")
            text(statusTextProperty) {}
          }

          div {
            classes = Seq("ui-carousel__indicators")
            ...
            foreachIndexed(indicatorItemsProperty) { (_, index) =>
              ...
              button((index + 1).toString) {
                classes = Seq("ui-carousel__indicator")
