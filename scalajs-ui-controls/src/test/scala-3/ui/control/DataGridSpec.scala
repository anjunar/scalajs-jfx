...
class DataGridSpec extends AnyFlatSpec with Matchers {
  ...
  "DataGrid SSR" should "render only the visible local grid cells" in {
    ...
    html should include("ui-data-grid")
    html should not include "ui-data-grid-header-slot"
    html should include regex "ui-data-grid-viewport[^>]*overflow: auto"
  ...
  it should "render unloaded remote ranges as placeholder cells" in {
    ...
    html should include("ui-data-grid-cell-loading")
  ...
  it should "render crawlable windows from its component cookie" in {
    ...
    val html    = Runtime.renderToString { cursor =>
      Runtime.mount(
        new CrawlTestRoot(
          cookieHeader = Some(
            s"ui-crawl-members-grid=${js.URIUtils.encodeURIComponent("5:4:")}"
          )
        ) {
  ...
  it should "reserve a full-width paging header in card-row units" in {
    ...
    html should include regex "ui-data-grid-header-slot[^>]*style=\"[^\"]*width: 100%"
  ...
  it should "page the content header away after the first page" in {
    ...
    html should include regex "ui-data-grid-header-slot[^>]*style=\"[^\"]*display: none"
