...
/** Text that reaches the SSR output unescaped.
  ...
  * escapes, which is right everywhere a component mounts text. Only [[jfx.core.document.HeadSink]]
  * builds these nodes, and it decides per entry whether the value is raw or escaped -- there is no
  * component that mounts one.
  */
