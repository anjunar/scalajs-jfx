...
describe("library components", () => {
  it("mounts the three registered components", () => {
    ...
    expect(html()).toBe(
      '<div class="ui-vbox"><div class="ui-hbox">' +
        '<button class="ui-button" type="button">Go</button>' +
        "</div></div>"
    );
  });

  it("rejects an unregistered component name", () => {
    // "combo-box" lives in scalajs-ui-controls, which neither runtime registers today.
