 ...
 *     a second copy of scalajs-ui-core cannot arise here -- it can at a stranger's.
...
beforeAll(() => {
  ...
  consumer = mkdtempSync(join(tmpdir(), "ui-consumer-"));
  ...
  // --legacy-peer-deps: @anjunar/scalajs-ui-core declares a peer on the CSS package,
  // which this probe deliberately does not install -- it imports no stylesheet,
  // and pulling @anjunar/scalajs-ui in behind it would test npm's network, not us.
...
describe("a packed install", () => {
  it("ships dist, and nothing that should have stayed home", () => {
    const installed = join(consumer, "node_modules", "@anjunar", "scalajs-ui-core");
  ...
  it("ships the bridge's types next to the linked, optimised bundle", () => {
    const installed = join(consumer, "node_modules", "@anjunar", "scalajs-ui-bridge");
...
describe("typechecking a consumer", () => {
  it("resolves both packages under --strict, including bridgeRuntime", () => {
    ...
    const source = [
      'import { button, classes, div, installRuntime, property, renderToString, text, vbox } from "@anjunar/scalajs-ui-core";',
      'import type { UiRuntime, Property, SsrResult } from "@anjunar/scalajs-ui-core";',
      'import { stubRuntime } from "@anjunar/scalajs-ui-core/stub";',
      'import { bridgeRuntime } from "@anjunar/scalajs-ui-bridge";',
      "",
      "const runtimes: readonly UiRuntime[] = [stubRuntime, bridgeRuntime];",
      "",
...
describe("one runtime, through two import routes", () => {
  it("reaches the same module instance and the same installed slot", () => {
    const probe = [
      ...
      'const bare = await import("@anjunar/scalajs-ui-core");',
      "",
      ...
      '// gave npm/scalajs-ui-demo two "installed" slots under Vite',
      "// (JAVASCRIPT_API.md §13). If the two were kept apart, the namespaces",
      "// would disagree below.",
      "const require = createRequire(import.meta.url);",
      'const real = realpathSync(require.resolve("@anjunar/scalajs-ui-core"));',
      "const direct = await import(pathToFileURL(real).href);",
      "",
      'const stub = await import("@anjunar/scalajs-ui-core/stub");',
...
describe("rendering from a packed install", () => {
  ...
  it("renders server-side against the stub", () => {
    const script = [
      'import { classes, div, installRuntime, property, renderToString, text, vbox } from "@anjunar/scalajs-ui-core";',
      'import { stubRuntime } from "@anjunar/scalajs-ui-core/stub";',
    ...
    expect(result.html).toContain('class="ui-vbox page"');
  });
  ...
  it("installs the bridge before a dependent module creates state at module load", () => {
    ...
    const script = [
      ...
      'import { div, renderToString, text } from "@anjunar/scalajs-ui-core";',
  ...
  it("refuses automatic installation over a different runtime", () => {
    const script = [
      'import { installRuntime, runtime } from "@anjunar/scalajs-ui-core";',
      'import { stubRuntime } from "@anjunar/scalajs-ui-core/stub";',
      "installRuntime(stubRuntime);",
      "let refused = false;",
      'try { await import("@anjunar/scalajs-ui-bridge"); }',
  ...
  it("renders server-side after automatic installation through a named import", () => {
    const script = [
      'import { classes, div, property, renderToString, text, vbox } from "@anjunar/scalajs-ui-core";',
      'import { bridgeRuntime } from "@anjunar/scalajs-ui-bridge";',
      page,
    ...
    expect(result.name).toBe("scalajs-ui-bridge");
    expect(result.status).toBe(200);
    expect(result.html).toContain("n=42");
  });
});
