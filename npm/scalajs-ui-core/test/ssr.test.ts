...
describe("renderToString", () => {
  ...
  it("renders the registered library components", async () => {
    ...
    expect(result.html).toBe(
      '<div class="ui-vbox"><button class="ui-button" type="button">Go</button></div>'
    );
  });
  ...
  it("marks blocks with comment anchors so hydration can find them", async () => {
    ...
    expect(result.html).toContain("<!--ui:Foreach:start-->");
    expect(result.html).toContain("<!--ui:Foreach:end-->");
