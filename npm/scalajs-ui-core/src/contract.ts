...
/** Undoes a subscription. Mirrors `ui.core.state.Disposable`. */
export interface Disposable {
  dispose(): void;
}

/** Mirrors `ui.core.state.ReadOnlyProperty`. */
...
/** Mirrors `ui.core.state.Property`. */
...
/** Mirrors `ui.core.state.ListProperty`. */
...
/** A DOM event as the runtime hands it over. Mirrors `ui.core.render.UiEvent`. */
...
export interface ScopeHandle {
   ...
   *
   * Mirrors `ui.core.dsl.DslLayer.child`: when `body` throws, the runtime
  ...
  /**
   * Mounts the `<head>` element. Mirrors `ui.core.layout.Head.head`.
   *
   ...
   * Mirrors `ui.core.document.DocumentHead.current`.
   */
  ...
  /** Mirrors `ui.core.layout.Condition.when`. */
  when(
    active: ReadOnlyProperty<boolean>,
    body: (scope: ScopeHandle) => void
  ): void;

  /** Mirrors `ui.core.statement.Foreach`. */
  forEach<T>(
  ...
  /**
   * Mirrors `ui.core.layout.FetchComponent.fetch`.
   *
   ...
   * live in TypeScript and the name-to-class table lives in `scalajs-ui-bridge`.
   */
  component(
    name: string,
...
/**
 * One element in the document head. Mirrors `ui.core.document.HeadEntry`.
 *
...
/**
 * The request-scoped head registry. Mirrors `ui.core.document.DocumentHead`.
 ...
 * `ui.core.i18n.MessageSourcePosition`. Always absent from a `RuntimeMessage` built by `i18n`/
...
/** One resolved placeholder value. Mirrors `ui.core.i18n.MessageArg`. */
 ...
 * `ui.core.i18n.RuntimeMessage`. Built by `i18n`/`i18nc` in `i18n.ts`, never by hand.
 */
 ...
 * `ui.core.i18n.CatalogEntry` (`MessageValue`'s `translations`, projected to plain strings --
 * `scalajs-ui-bridge` is where `LocalizedPattern` and `MessageValue.state` exist; a TS-authored catalog
