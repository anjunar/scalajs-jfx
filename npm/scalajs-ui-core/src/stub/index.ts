 ...
 * The stub runtime.
 *
 * It implements the same contract as `scalajs-ui-bridge` on top of a small host
...
class StubScope implements ScopeHandle {
  ...
  private i18nUnsupported(): never {
    throw new Error(
      "The stub runtime has no i18n support. Test i18n-dependent code against " +
        "@anjunar/scalajs-ui-bridge, or mount an i18n-independent tree under the stub."
    );
  }
  ...
  private range(label: string): { scope: StubScope; clear: () => void } {
    const start = this.doc.createComment(`ui:${label}:start`);
    const end = this.doc.createComment(`ui:${label}:end`);
    this.insert(start);
    this.insert(end);
...
export class StubRuntime implements UiRuntime {
  readonly name = "stub";
