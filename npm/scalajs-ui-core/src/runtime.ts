...
let installed: UiRuntime | null = null;
 ...
 */
export function installRuntime(runtime: UiRuntime): void {
  if (installed !== null && installed !== runtime) {
    throw new Error(
      `A UI runtime is already installed ("${installed.name}"). Installing a ` +
...
export function runtime(): UiRuntime {
  if (installed === null) {
    throw new Error(
      'No UI runtime installed. Import "@anjunar/scalajs-ui-bridge" ' +
