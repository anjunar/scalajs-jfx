 ...
 *
 * Two shapes, the same as on the Scala side (`ui.core.state.ListDataSource`):
 *
 *  - a `ListProperty<T>` from `@anjunar/scalajs-ui-core` -- a local, fully materialized
 ...
 * bridge, which builds a `ui.core.remote.RemoteListProperty` around it.
 */
import type { ListProperty } from "@anjunar/scalajs-ui-core";

/** One sort term. Mirrors `ui.core.remote.RemoteSort`. */
export interface SortSpec {
  readonly field: string;
  readonly ascending: boolean;
}

/** One loaded page. Mirrors `ui.core.remote.RemotePage`. */
