...
- `src/designs.css` interprets the Constitution's landing slots for each design: Atlas's register, Flora's generous surfaces, Terra's editorial opening and Ember's framed tools. The typography, semantic color roles, spacing and motion come from the shared tokens. See [CONSTITUTION.md](CONSTITUTION.md) for the mapping and checked scope.
- `src/presentation.css` establishes the `height: 100%` chain and native snap positions for ten slides. `src/presentation.mjs` adds a small desktop wheel/keyboard controller. Longer slides scroll internally; touch, nested widgets, native links and focus retain their own behavior. See [FULLPAGE.md](FULLPAGE.md) for the interaction contract and verification.
