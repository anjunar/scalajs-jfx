...
| --- | --- |
| Form übernimmt Last. | A clear title, named navigation, section headings, readable prose measures, code scroll regions and a semantic comparison table. The header stays in normal flow so it cannot cover focused controls or enlarged text. |
...
- **Atlas:** compact numbered register, clear rules and restrained tool surfaces.
- **Flora:** light serif titles, generous intervals and soft surfaces around actual working examples.
- **Terra:** split editorial opening, strong serif hierarchy, brass wayfinding, green actions and open specimen rules.
- **Ember:** strong sans titles, compact monospace context, copper wayfinding and framed working areas.

Decorative accents use `--color-accent`; navigation/actions use `--color-interactive`; success/error colors describe actual states. The typography and spacing scale is supplied by each complete shared design package. No alternative DOM, content order or design-specific JavaScript is introduced.
