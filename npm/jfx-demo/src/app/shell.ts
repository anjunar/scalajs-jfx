...
import { anchor, attr, button, classes, div, element, isBrowser, locale, onClick, span, text } from "@anjunar/jfx-core";
import { router, routerLink, type RouteDefinition, type RouterConfig } from "@anjunar/jfx-router";
...
export function appShell(routes: readonly RouteDefinition[], config: RouterConfig = {}): void {
  ...
  router(routes, config, outlet => {
    div(() => {
      ...
      header(() => {
        classes("site-header");
        routerLink("/", "", {}, () => { classes("brand"); text("JFX API"); });
        div(() => {
      ...
      footer(() => {
        ...
        externalLink("GitHub", "https://github.com/anjunar/scalajs-jfx");
        text("v3.0.2");
      });
    });
  });
