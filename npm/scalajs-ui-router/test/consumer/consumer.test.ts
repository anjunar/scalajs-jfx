 ...
 *  2. `@anjunar/scalajs-ui-router` ships `dist` and its types, not `src`/`test`.
...
beforeAll(() => {
  ...
  consumer = mkdtempSync(join(tmpdir(), "scalajs-ui-router-consumer-"));
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        name: "scalajs-ui-router-consumer-probe",
        private: true,
        version: "0.0.0",
        type: "module",
        dependencies: {
          ...
          "@anjunar/scalajs-ui-router": fileSpecifier(routerTarball),
        },
      },
      null,
      2
  ...
  // --legacy-peer-deps: scalajs-ui-core declares a peer on the CSS package this probe
...
describe("a packed install", () => {
  it("ships dist and types, and nothing that should have stayed home", () => {
    const installed = join(consumer, "node_modules", "@anjunar", "scalajs-ui-router");
...
describe("typechecking a consumer", () => {
  it("resolves all three packages under --strict", () => {
    ...
    const source = [
      'import { div, installRuntime, renderToString, text } from "@anjunar/scalajs-ui-core";',
      'import type { SsrResult } from "@anjunar/scalajs-ui-core";',
      'import { bridgeRuntime } from "@anjunar/scalajs-ui-bridge";',
      'import { errorRoute, router, routerLink, routerOutlet, view } from "@anjunar/scalajs-ui-router";',
      'import type { RouteContext, RouteDefinition } from "@anjunar/scalajs-ui-router";',
      "",
...
describe("rendering a route table from a packed install", () => {
  it("renders the matched route and an error route's status against the bridge", () => {
    const script = [
      'import { div, installRuntime, renderToString, text } from "@anjunar/scalajs-ui-core";',
      'import { bridgeRuntime } from "@anjunar/scalajs-ui-bridge";',
      'import { errorRoute, router, view } from "@anjunar/scalajs-ui-router";',
      "installRuntime(bridgeRuntime);",
      "const routes = [",
