 ...
 * navigating anchor.
 *
 * Each of the three is a registry entry in `scalajs-ui-bridge` (`RouterFactories.scala`).
 * The one non-obvious crossing is `load`: `ui.router.Route` wants
...
import type { ReadOnlyProperty, ScopeHandle } from "@anjunar/scalajs-ui-core";
 ...
 * `ui.router.Route` gives Scala between `Future.successful` and a real `Future`.
 *
