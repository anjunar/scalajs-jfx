...
```ts
import type { MediaUploader } from "@anjunar/jfx-editor";

const mediaUploader: MediaUploader = {
  async upload(file, signal) {
    const form = new FormData();
    form.append("image", file);
    // This example route belongs to the consuming application, not the JFX library.
...
contract. JFX forms intercept submit in JavaScript; configure the actual action, storage and
