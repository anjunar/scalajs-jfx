...
import { installRuntime, mount, property, renderToString, resetRuntime, hydrate } from "@anjunar/jfx-core";
...
function setup(markdown = "Before", options: EditorOptions = {}) {
  ...
  const surface = root.querySelector(".jfx-editor__surface") as HTMLElement & { __lexicalEditor: LexicalEditor };
...
describe("uploads", () => {
  ...
  it("moves an internal image without uploading or duplicating it", async () => {
    ...
    root.querySelector(".jfx-editor__surface p:last-child")!.dispatchEvent(drop);
  ...
  it("uploads through the picker and retries in the same dialog after failure", async () => {
    ...
    const confirm = document.querySelector(".jfx-dialog__button--primary") as HTMLButtonElement;
    confirm.click();
...
describe("server and form contract", () => {
  it("preserves textarea input entered before hydration", async () => {
    ...
    expect(root.querySelector(".jfx-editor__surface")?.textContent).toBe("Typed before hydration");
  });
  it("renders width/title without uploading and hydrates the same reference", async () => {
    ...
    const app = await hydrate(root, build);
    disposers.push(() => app.dispose());
    expect(root.querySelector(".jfx-editor__surface img")?.getAttribute("title")).toBe("Garden");
    expect(root.querySelector(".jfx-editor__surface img")?.getAttribute("width")).toBe("320");
    expect(upload).not.toHaveBeenCalled();
  });
