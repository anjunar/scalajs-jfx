 ...
 *  2. `@anjunar/scalajs-ui-viewport` ships `dist` and its types, not `src`/`test`.
...
beforeAll(() => {
  ...
  consumer = mkdtempSync(join(tmpdir(), "scalajs-ui-viewport-consumer-"));
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        name: "scalajs-ui-viewport-consumer-probe",
        private: true,
        version: "0.0.0",
        type: "module",
        dependencies: {
...
describe("a packed install", () => {
  it("ships dist and types, and nothing that should have stayed home", () => {
    const installed = join(consumer, "node_modules", "@anjunar", "scalajs-ui-viewport");
...
describe("typechecking a consumer", () => {
  it("resolves all three packages under --strict", () => {
    ...
    const source = [
      'import { button, div, installRuntime, onClick, property, renderToString, text, when } from "@anjunar/scalajs-ui-core";',
      'import type { SsrResult } from "@anjunar/scalajs-ui-core";',
      'import { bridgeRuntime } from "@anjunar/scalajs-ui-bridge";',
      'import { floatingWindow, notify, overlay, viewport } from "@anjunar/scalajs-ui-viewport";',
      'import type { NotificationOptions, OverlayOptions, WindowOptions } from "@anjunar/scalajs-ui-viewport";',
      "",
...
describe("rendering a viewport from a packed install", () => {
  it("renders a notification and a window against the bridge", () => {
    const script = [
      ...
      "  viewportHost: result.html.includes(\"scalajs-ui-viewport\"),",
      "  notification: result.html.includes(\"scalajs-ui-viewport-notification--success\"),",
