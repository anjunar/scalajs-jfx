...
beforeAll(() => {
  consumer = mkdtempSync(join(tmpdir(), "scalajs-ui-webauthn-consumer-"));
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: "scalajs-ui-webauthn-consumer",
      private: true,
      type: "module",
      dependencies: { "@anjunar/scalajs-ui-webauthn": fileSpecifier(tarball) },
    }),
  );
...
describe("packed install", () => {
  it("ships the public build and omits sources and tests", () => {
    const installed = join(consumer, "node_modules", "@anjunar", "scalajs-ui-webauthn");
  ...
  it("resolves the public API under strict TypeScript", () => {
    ...
    writeFileSync(
      ...
      [
        'import { Base64Url, CredentialMediationRequirement, authenticateJson, isAvailable } from "@anjunar/scalajs-ui-webauthn";',
