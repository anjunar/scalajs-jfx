...
export default defineConfig(({ isSsrBuild }) => ({
  ...
  plugins: [{
    ...
    async load(id) {
      ...
      for (const name of ["scalajs-ui-core", "scalajs-ui-bridge", "scalajs-ui"]) {
        if (JSON.parse(await read(`npm/${name}/package.json`)).version !== version) throw Error(`Version mismatch: ${name}`);
      }
      for (const file of ["build.sbt", "project/plugins.sbt", "project/build.properties", "npm/scalajs-ui-landing/starters/Counter.scala", "npm/scalajs-ui-landing/src/counter.mjs"]) this.addWatchFile(`${root}/${file}`);
      const scalaSource = await read("npm/scalajs-ui-landing/starters/Counter.scala");
      const tsSource = await read("npm/scalajs-ui-landing/src/counter.mjs");
      return `export default ${JSON.stringify({version, scalaVersion, scalaJsVersion, sbtVersion, scalaSource, tsSource})};`;
    }
  }],
  resolve: { dedupe: ["@anjunar/scalajs-ui-core", "@anjunar/scalajs-ui-controls", "@anjunar/scalajs-ui-forms", "@anjunar/scalajs-ui-editor", "@anjunar/scalajs-ui-bridge"] },
