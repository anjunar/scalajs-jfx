...
function fixture({ url = 'https://ui.example/de/?design=terra#main', saved, blocked = false } = {}) {
  ...
  if (saved) win.localStorage.setItem('scalajs-ui.landing.view', saved);
...
test('presentation is restored before paint; valid URL choices override storage', () => {
  for (const [url, saved, expected] of [
    ['https://ui.example/', 'presentation', true],
    ['https://ui.example/?view=reading', 'presentation', false],
    ['https://ui.example/?view=presentation', 'reading', true],
    ['https://ui.example/?view=unknown', 'unknown', false],
  ]) {
...
test('view changes persist and retain locale, appearance and fragment links', () => {
  ...
  assert.equal(f.win.localStorage.getItem('scalajs-ui.landing.view'), 'presentation');
  ...
  f.change('reading'); assert.equal(f.win.localStorage.getItem('scalajs-ui.landing.view'), 'reading');
  f.close();
});
