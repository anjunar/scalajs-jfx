...
export function localizePage(html, locale) {
  ...
  function visit(node, skip = false) {
    ...
    const preserve = skip || ["script", "style", "pre", "code"].includes(node.tagName) || ["counter-root", "table-root"].includes(attrs.id) || ["ui-form", "ui-table-view", "scalajs-ui-editor"].some(c => attrs.class?.split(" ").includes(c));
