...
import { button, onClick } from "@anjunar/scalajs-ui-core";
...
beforeAll(() => {
  ...
  // comboBox's dropdown is a scalajs-ui-controls TableView, which observes its
  // viewport size in the browser; jsdom ships neither observer. A no-op pair
  // is enough for mount/hydrate to run -- same polyfill scalajs-ui-controls' own
  // smoke test uses.
  const noop = class {
    observe(): void {}
    unobserve(): void {}
...
describe("form + input", () => {
  it("validates native date input values and boolean model values", () => {
    ...
    let handle!: import("@anjunar/scalajs-ui-core").FormHandle;
  ...
  it("returns a handle for validation and binding diagnostics", () => {
    ...
    expect(root.querySelector(".ui-input-container__errors")?.textContent).toContain(
      "Rejected by server"
    );
    handle!.clearErrors();
    expect(root.querySelector(".ui-input-container__errors")?.textContent ?? "").toBe("");
    app.dispose();
  });
  ...
  it("attaches schema validators and reports errors through inputContainer", () => {
    ...
    expect(root.querySelector(".ui-input-container__errors")?.textContent ?? "").not.toBe("");
...
describe("fieldSet", () => {
  it("forwards errors and binding diagnostics through a nested dynamic form", () => {
    ...
    try {
      ...
      expect(root.querySelector(".ui-input-container__errors")?.textContent).toContain("Grouped error");
      handle.clearErrors();
      expect(root.querySelector(".ui-input-container__errors")?.textContent ?? "").toBe("");
      app.dispose();
...
describe("arrayForm", () => {
  it("propagates nested server errors and binding diagnostics through array items", () => {
    ...
    try {
      ...
      expect(root.querySelector(".ui-input-container__errors")?.textContent).toContain("Nested server error");
      handle.clearErrors();
      expect(root.querySelector(".ui-input-container__errors")?.textContent ?? "").toBe("");
      model.rows.clear();
      expect(handle.validateBindings()).toEqual([]);
...
describe("comboBox", () => {
  it("selects an item and updates the bound model property", () => {
    ...
    const trigger = root.querySelector(".ui-combo-box") as HTMLElement;
    trigger.dispatchEvent(new MouseEvent("click", { bubbles: true }));

    const option = root.querySelector(".ui-combo-box__item") as HTMLElement;
  ...
  it("opens its dropdown from a bare click after hydration", async () => {
    ...
    expect(root.querySelector(".ui-combo-box__item")).not.toBeNull();
    app.dispose();
  });
});
