 ...
 *  2. `@anjunar/scalajs-ui-forms` ships `dist` and its types, not `src`/`test`.
...
beforeAll(() => {
  ...
  consumer = mkdtempSync(join(tmpdir(), "scalajs-ui-forms-consumer-"));
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        name: "scalajs-ui-forms-consumer-probe",
        private: true,
        version: "0.0.0",
        type: "module",
        dependencies: {
...
describe("a packed install", () => {
  it("ships dist and types, and nothing that should have stayed home", () => {
    const installed = join(consumer, "node_modules", "@anjunar", "scalajs-ui-forms");
...
describe("typechecking a consumer", () => {
  it("resolves all packages under --strict", () => {
    ...
    const source = [
      ...
      'import { form, input, notBlank } from "@anjunar/scalajs-ui-forms";',
      'import type { FormModel, FormOptions } from "@anjunar/scalajs-ui-forms";',
      "",
