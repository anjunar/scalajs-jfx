 ...
 *
 * `ui.forms.Form`/`SubForm` derive a control's validators from annotations a
 ...
 * `{ name: "ui.forms.validators.NotNull", parameters: {} }`, and
 * `FormFactories.schemaFrom` (in `scalajs-ui-bridge`) turns it into a real
...
function spec(name: string, parameters: Record<string, unknown> = {}): ValidatorSpec {
  ...
  return { name: `ui.forms.validators.${name}`, parameters: out };
}
