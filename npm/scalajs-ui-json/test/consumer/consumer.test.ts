...
beforeAll(() => {
  consumer = mkdtempSync(join(tmpdir(), "scalajs-ui-json-consumer-"));
  const tarballs = join(consumer, "tarballs"); mkdirSync(tarballs);
  const tarball = pack(packageRoot, tarballs);
  const coreTarball = pack(join(repoRoot, "npm", "scalajs-ui-core"), tarballs);
  writeFileSync(join(consumer, "package.json"), JSON.stringify({ name: "probe", private: true, type: "module", dependencies: { "@anjunar/scalajs-ui-json": fileSpecifier(tarball), "@anjunar/scalajs-ui-core": fileSpecifier(coreTarball) } }));
...
describe("packed install", () => {
  it("ships only the public build", () => {
    const installed = join(consumer, "node_modules", "@anjunar", "scalajs-ui-json");
