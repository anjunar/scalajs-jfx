 ...
 * variable inside `@anjunar/scalajs-ui-core`. That is safe only as long as there is
 * exactly one *instance* of that module. Two instances are two slots, and the
 * second one has never seen `installRuntime` -- the symptom is "No UI runtime
 ...
 * module. A bundler that duplicated scalajs-ui-core would duplicate the literal with
...
const MARKER = "A UI runtime is already installed";
...
async function devServer() {
  ...
  try {
    ...
    // /controls/table pulls in @anjunar/scalajs-ui-controls; if that package dragged
    // its own copy of scalajs-ui-core, the runtime marker count above would rise and
    ...
    check(
      ...
      controls.status === 200 && controlsHtml.includes("ui-table-view"),
      `status ${controls.status}, table markup ${controlsHtml.includes("ui-table-view")}`
    );
    check(
      "dev server did not log a runtime-installation fault",
      !stderr.join("").includes("No UI runtime installed"),
