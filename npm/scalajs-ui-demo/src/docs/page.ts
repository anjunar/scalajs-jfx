 ...
 */
import { anchor, article, attr, classes, disposeWith, div, documentHead, heading, meta as metaTag, paragraph, span, text, title as titleTag } from "@anjunar/scalajs-ui-core";
...
export function docPage(meta: DocPageMeta, body: () => void): void {
  ...
  article(() => {
    ...
    if (head !== null) {
      ...
      const updateHead = () =>
        pageHead.set(
          titleTag(`${localizedTitle.get} · Scala JS UI 1.0 demo`),
...
function runtimeInsights(): void {
  div(() => {
    ...
    insight(0, "Runtime", "Shared engine", "The TypeScript facade delegates to the installed Scala.js UI runtime.");
