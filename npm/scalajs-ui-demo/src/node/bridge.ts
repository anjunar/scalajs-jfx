 ...
 * controls/, forms/ and viewport/, plus the six @anjunar/scalajs-ui-core pages
 * node/stub.ts also renders. Run with `npm run demo:bridge`. Needs
 * @anjunar/scalajs-ui-bridge linked -- `sbt --server "scalajs-ui-bridge/fullLinkJS"`
 * from the repo root.
 *
 * It uses the build-safe page manifest rather than `app/catalog.ts`: every
 * doc.ts closes over a `?ui-code` import that only resolves inside Vite's
 ...
 */
import { property, renderToString } from "@anjunar/scalajs-ui-core";
