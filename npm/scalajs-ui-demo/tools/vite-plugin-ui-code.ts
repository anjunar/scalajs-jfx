/**
 * Answers imports of the form `./page.ts?ui-code` (the whole file) or
 * `./page.ts?ui-code=composer` (the region marked `// #region composer` /
...
const QUERY_PREFIX = "?ui-code";

export function uiCode(): Plugin {
  return {
    name: "ui-code",
...
function extractRegion(source: string, region: string | null): string {
  ...
  if (startIndex === -1) {
    throw new Error(`ui-code: region "${region}" not found`);
  }
  ...
  if (endIndex === -1) {
    throw new Error(`ui-code: region "${region}" has no matching #endregion`);
  }
