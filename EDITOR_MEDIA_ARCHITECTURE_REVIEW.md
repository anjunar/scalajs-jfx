...
Die aktualisierte Begleitbibliothek wurde ausschließlich als `scalajs-lexical` 1.4.0-SNAPSHOT lokal veröffentlicht. Für unabhängige CI und Distribution ist eine koordinierte Maven-Veröffentlichung erforderlich. Der konkrete Upload-Endpunkt, dauerhafte Speicherung, Backend-Validierung und die Backend-Unterstützung für `{width=N}` bleiben Aufgaben der konsumierenden Anwendung. Ein vollständiger Non-JS-Multipart-Upload ist weiterhin ein separater Folgeschritt.
