...
object ButtonPageTranslations {
  val entries: Seq[CatalogEntry] = Seq(
    ...
    de(
      i18n"In Scala JS UI 1.0 the action stays visible in the template: label, event, and surrounding context sit next to each other. That keeps simple buttons easy to read and leaves room for more complex workflows later.",
      "In Scala JS UI 1.0 bleibt die Aktion im Template sichtbar: Beschriftung, Ereignis und umgebender Kontext stehen direkt beieinander. Dadurch bleiben einfache Buttons leicht lesbar und es bleibt Raum für komplexere Abläufe."
    ),
