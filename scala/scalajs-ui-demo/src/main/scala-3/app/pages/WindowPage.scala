    ...
    showcasePage(i18n"Window & viewport", i18n"The architecture of space.") {
      vbox {
        ...
        sectionIntro(
          ...
          i18n"Notifications, windows, and overlays need a central order. Scala JS UI 1.0 gathers those concerns in the viewport so pages do not have to manage global UI layers on their own."
        )
        ...
        componentShowcase(
          ...
          vbox {
            style { gap = "12px" }
            button(i18n"Info notification") {
              onClick { _ =>
                Viewport.notify(
                  i18nRuntime.resolveNow(i18n"Silence is the origin of every form."),
                  NotificationKind.Info
                )
              }
            }
            button(i18n"Success notification") {
              onClick { _ =>
                Viewport.notify(
                  i18nRuntime.resolveNow(i18n"The structure is now sound."),
                  NotificationKind.Success
                )
              }
            }
            button(i18n"Warning notification") {
              onClick { _ =>
                Viewport.notify(
                  i18nRuntime.resolveNow(i18n"Warning: the form may be hardening."),
                  NotificationKind.Warning
                )
              }
            }
            button(i18n"Error notification") {
              onClick { _ =>
                Viewport.notify(
                  i18nRuntime.resolveNow(i18n"A crack in the foundation was discovered."),
        ...
        componentShowcase(
          ...
          button(i18n"Open window") {
            onClick { _ =>
              Viewport.addWindow(
                i18nRuntime.resolveNow(i18n"A room for thoughts"),
                widthPx = 400,
                heightPx = 300
              ) {
                vbox {
                  ...
                  button(i18n"Confirm note") {
                    onClick { _ =>
                      Viewport.notify(
                        i18nRuntime.resolveNow(i18n"The note in the window was confirmed."),
                        NotificationKind.Success
                      )
                    }
                  }
