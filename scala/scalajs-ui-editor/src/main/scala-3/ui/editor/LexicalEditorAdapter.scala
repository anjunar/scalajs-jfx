...
import ui.editor.plugins.{DefaultDialogService, EditorPlugin}
import lexical.*
...
/** Owns the complete browser-side Lexical lifecycle for one [[Editor]].
  *
  * The adapter has no form or UI control state. Markdown enters and leaves through callbacks;
...
private[editor] final class LexicalEditorAdapter(
  ...
  def mount(markdown: String, editable: Boolean): Unit =
    if (editor == null && !closed) {
      ...
      coordinator.install()
      plugins
        .collect { case image: ui.editor.plugins.ImagePlugin => image }
