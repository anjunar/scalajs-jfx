...
import ui.core.state.{Disposable, ListDataSource, ListProperty, Property}
...
class TableRowSpec extends AnyFlatSpec with Matchers {
  ...
  it should "retain overlapping rows and dispose rows and their observers on factory replacement" in {
    ...
    mounted(source) {
      ...
      cursor.collectHtml() should include("ui-table-cell")
    }
  }
  ...
  it should "refresh custom snapshots and stop rendering after disposal" in {
    ...
    mounted(source) {
      ...
      cursor.collectHtml() should not include "ui-table-cell"
      renders shouldBe 2
    }
    saved.refresh()
