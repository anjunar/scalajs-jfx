...
/** The one component every `Build` mounts under.
  ...
  * (`JfxRuntimeBridge.renderToString`'s `options.document`, `.hydrate`'s `Document` branch). A
  ...
  * content directly, with no enclosing `html(...)` of its own; see `jfx-demo`'s `app/document.ts`.
  *
