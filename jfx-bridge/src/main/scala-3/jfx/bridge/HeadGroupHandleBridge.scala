...
/** The JS projection of `DocumentHead.Handle`. Mirrors `contract.ts`'s `HeadGroupHandle`: a group of
  * head entries replaced as a whole on every [[set]], for an entry that changes without its owning
